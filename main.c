...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
//  500 X 2500 initial setup
//  FOR PANNING AXIS AKA CHANNEL 1
//  -90 deg is 500 usec
//  0 deg is 1500 usec
//  90 deg is 2500 usec
//  FOR TILT AXIS AKA CHANNEL 2
//  -45 deg is 2000 usec
//  0 deg is 1500 usec
//  45 deg is 1000 usec

//  150 is 2 x 100 in Parallel + 100 in Series
//  80 is to be determined (80 = 80?)
//  75 is (4 x 100 in Parallel + 2 x 100 in Parallel) in Series
  ...
  {
    // SERVO CODE
	  while(TIM2_Ch1_DCVAL < 2500) { // MOTOR 1 CONTROL
	  TIM2_Ch1_DCVAL +=  (20); // SIGNAL JUMP
      TIM2_Ch2_DCVAL -= (20); // SIGNAL JUMP
      TIM2->CCR1 = TIM2_Ch1_DCVAL; //SET REGISTER VALUES
      TIM2->CCR2 = TIM2_Ch2_DCVAL; //SET REGISTER VALUES
      HAL_Delay(50); // TIME DELAY ON PULSE SHIFT
//      sprintf((char*)txd_msg_buffer, " PAN: %d  TILT: %d \r\n ", TIM2_Ch1_DCVAL, TIM2_Ch2_DCVAL);
//     HAL_UART_Transmit(&huart6,txd_msg_buffer,strlen((char*)txd_msg_buffer),1000);


	  }
    while(TIM2_Ch1_DCVAL > 500) { // MOTOR 2 CONTROL
      TIM2_Ch1_DCVAL -= (20); // SIGNAL JUMP
      TIM2_Ch2_DCVAL += (20); // SIGNAL JUMP
      TIM2->CCR1 = TIM2_Ch1_DCVAL; //SET REGISTER VALUES
      TIM2->CCR2 = TIM2_Ch2_DCVAL; //SET REGISTER VALUES
...
//     HAL_UART_Transmit(&huart6,txd_msg_buffer,strlen((char*)txd_msg_buffer),1000);
    ...
    sprintf((char*)txd_msg_buffer, " X-Cords: %d  Y-Cords: %d \r\n ", ADC_CH14, ADC_CH9);
    ...
    while(us100_Rx_flag ==(00)) {}; 											// wait for the Flag to be set by the Interrupt Callback routine
		...
		distance = (us100_buffer[0] << 8) + us100_buffer[1];  					// convert the two individual bytes into a single 16 bit quantity
		...
		us100_Rx_flag = 00;														// clear the flag
    /* USER CODE END WHILE */
